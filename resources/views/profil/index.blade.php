...
<head>
    ...
    <style>
        ...
        body:not(.theme-dark) .bg-gradient-light {
            ...
            background: linear-gradient(135deg, rgba(20, 184, 166, 0.08) 0%, transparent 34%),
        ...
        body.theme-dark .bg-gradient-dark { position: fixed; inset: 0; z-index: -3; background: linear-gradient(135deg, #07111f 0%, #0f172a 58%, #14312f 100%); }
...
<body class="antialiased min-h-screen relative pb-12 flex flex-col custom-scroll {{ $isStaff ? 'theme-dark selection:text-white selection:bg-'.$accent.'-500' : 'selection:bg-teal-600 selection:text-white' }}">
    ...
    @else
        <nav class="w-full bg-white/80 backdrop-blur-md border-b border-slate-200 sticky top-0 z-50">
            <div class="w-full px-6 md:px-10 py-4 flex justify-between items-center">
                <a href="{{ url('/dashboard') }}" class="flex items-center gap-3 text-[10px] md:text-xs font-black text-slate-500 hover:text-teal-700 uppercase tracking-widest transition-colors group">
                    <div class="w-8 h-8 rounded-full border border-slate-200 flex items-center justify-center group-hover:border-teal-600 group-hover:bg-teal-50 transition-all">
    ...
    <main class="w-full px-6 md:px-10 mt-4 flex-grow flex flex-col">
        ...
        <div class="text-center mb-10">
            ...
            <p class="{{ $isStaff ? 'text-slate-400' : 'text-slate-500' }} font-medium text-xs md:text-sm mt-3">Data diri Anda aman di sistem ALBRK.SHOECARE.</p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 xl:gap-8 items-stretch max-w-7xl mx-auto w-full">
            ...
            <div class="glass-card rounded-[2.5rem] p-8 md:p-10 flex flex-col h-full relative overflow-hidden group">
                <div class="flex-grow flex flex-col items-center">
                    <div class="w-32 h-32 rounded-full border-4 {{ $isStaff ? 'border-slate-700 bg-slate-800' : 'border-white bg-teal-700' }} shadow-xl overflow-hidden text-white flex items-center justify-center font-black text-5xl mb-4 relative group-hover:scale-105 transition-all duration-500">
                    ...
                    <span class="{{ $isStaff ? 'bg-teal-500/20 text-teal-300 border-teal-500/30' : 'bg-teal-50 text-teal-700 border-teal-100' }} px-4 py-1.5 rounded-full text-[10px] uppercase font-black tracking-widest border">
                    ...
                    <form action="{{ route('profil.updateFoto') }}" method="POST" enctype="multipart/form-data" id="form-upload" class="w-full text-left">
                        ...
                        <div class="relative w-full group/dropzone">
                            <div class="w-full {{ $isStaff ? 'bg-slate-800/50 hover:border-'.$accent.'-500' : 'bg-slate-50 hover:border-teal-600' }} border-2 border-dashed border-slate-700 rounded-2xl p-5 flex flex-col items-center justify-center transition-all cursor-pointer">
                ...
                <form action="{{ route('profil.update') }}" method="POST" class="flex flex-col flex-grow">
                    ...
                    <div class="space-y-5 flex-grow">
                        <div>
                            ...
                            <input type="text" name="nama" value="{{ auth()->user()->nama }}" required class="w-full {{ $isStaff ? 'bg-slate-800/50 border-slate-700 text-white focus:border-'.$accent.'-500' : 'bg-slate-50 border-slate-200 focus:border-teal-600' }} border px-5 py-4 rounded-xl text-xs font-bold outline-none transition-all">
                        </div>
                        <div>
                            <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 ml-1">Email</label>
                            <input type="email" name="email" value="{{ auth()->user()->email }}" required class="w-full {{ $isStaff ? 'bg-slate-800/50 border-slate-700 text-white focus:border-'.$accent.'-500' : 'bg-slate-50 border-slate-200 focus:border-teal-600' }} border px-5 py-4 rounded-xl text-xs font-bold outline-none">
                        </div>
                        <div>
                            <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 ml-1">No. WhatsApp</label>
                            <input type="text" name="no_hp" value="{{ auth()->user()->no_telp }}" class="w-full {{ $isStaff ? 'bg-slate-800/50 border-slate-700 text-white focus:border-'.$accent.'-500' : 'bg-slate-50 border-slate-200 focus:border-teal-600' }} border px-5 py-4 rounded-xl text-xs font-bold outline-none">
                        </div>
                        <div>
                            <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 ml-1">Alamat</label>
                            <textarea name="alamat" rows="2" class="w-full {{ $isStaff ? 'bg-slate-800/50 border-slate-700 text-white focus:border-'.$accent.'-500' : 'bg-slate-50 border-slate-200 focus:border-teal-600' }} border px-5 py-4 rounded-xl text-xs font-bold outline-none resize-none">{{ auth()->user()->alamat }}</textarea>
                        </div>
                    </div>
                ...
                <form action="{{ route('profil.updatePassword') }}" method="POST" class="flex flex-col flex-grow">
                    ...
                    <div class="space-y-5 flex-grow">
                        <div>
                            ...
                            <input type="password" name="current_password" required class="w-full {{ $isStaff ? 'bg-slate-800/50 border-slate-700 text-white focus:border-'.$accent.'-500' : 'bg-slate-50 border-slate-200 focus:border-teal-600' }} border px-5 py-4 rounded-xl text-xs font-bold outline-none">
                        </div>
                        <div>
                            <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 ml-1">Password Baru</label>
                            <input type="password" name="new_password" required class="w-full {{ $isStaff ? 'bg-slate-800/50 border-slate-700 text-white focus:border-'.$accent.'-500' : 'bg-slate-50 border-slate-200 focus:border-teal-600' }} border px-5 py-4 rounded-xl text-xs font-bold outline-none">
                        </div>
                        <div>
                            <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 ml-1">Konfirmasi Password</label>
                            <input type="password" name="new_password_confirmation" required class="w-full {{ $isStaff ? 'bg-slate-800/50 border-slate-700 text-white focus:border-'.$accent.'-500' : 'bg-slate-50 border-slate-200 focus:border-teal-600' }} border px-5 py-4 rounded-xl text-xs font-bold outline-none">
                        </div>
                    </div>
