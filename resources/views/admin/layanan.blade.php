                    ...
                    <table class="w-full text-left border-collapse min-w-[700px] md:min-w-0">
                        ...
                        <tbody class="divide-y divide-slate-800/50 text-sm">
                            ...
                            <tr class="hover:bg-slate-800/30 transition-all group">
                                <td class="px-8 py-5">
                                    @if($l->gambar)
                                        <img src="{{ route('media.public', ['path' => $l->gambar]) }}" class="w-14 h-14 md:w-16 md:h-16 object-cover rounded-[1rem] shadow-md border border-slate-700">
                                    @else
                                ...
                                <td class="px-8 py-5 text-right whitespace-nowrap">
                                    <div class="flex justify-end gap-3">
                                        <button onclick="openEditModal('{{ $l->id_layanan }}', '{{ $l->nama_layanan }}', '{{ $l->harga }}', '{{ $l->deskripsi }}', '{{ $l->gambar ? route('media.public', ['path' => $l->gambar]) : '' }}')" class="w-10 h-10 rounded-full bg-slate-800 text-slate-400 hover:bg-{{ $accent }}-600 hover:text-white border border-slate-700 hover:border-transparent transition-all flex items-center justify-center shadow-lg active:scale-90">
