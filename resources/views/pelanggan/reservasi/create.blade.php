...
<body class="antialiased selection:bg-indigo-600 selection:text-white flex flex-col h-screen overflow-hidden relative">
    ...
    <header class="glass-nav px-6 md:px-12 py-4 flex justify-between items-center shrink-0 z-40 relative">
        <div class="flex items-center gap-3 md:gap-4">
            <a href="{{ url('/dashboard') }}" class="w-9 h-9 md:w-10 md:h-10 rounded-full bg-white border border-slate-200 text-slate-400 hover:text-indigo-600 hover:bg-blue-50 transition-all flex items-center justify-center shadow-sm group active:scale-95">
            ...
            <h1 class="block font-black text-xl md:text-2xl uppercase tracking-tighter italic text-slate-900 leading-tight">
                ALBRK.<span class="text-indigo-600">MEMBER</span>
            </h1>
        </div>
        ...
        <div class="max-w-5xl mx-auto">
            ...
            <div class="mb-8 md:mb-10 text-center px-2">
                <div class="inline-flex items-center gap-2 bg-blue-50 border border-blue-100 px-4 py-1.5 rounded-full mb-4">
                    ...
                    <p class="text-[8px] md:text-[9px] font-black text-indigo-600 uppercase tracking-[0.4em]">Order & Booking</p>
                </div>
            ...
            <form id="formReservasi" action="{{ route('reservasi.store') }}" method="POST" class="glass-card rounded-[2rem] md:rounded-[3rem] p-6 md:p-10 lg:p-14 space-y-10 md:space-y-12">
                ...
                <div>
                    <h3 class="font-black text-base md:text-lg uppercase tracking-widest mb-6 flex items-center gap-3 text-slate-900">
                        <span class="bg-indigo-600 text-white w-7 h-7 md:w-8 md:h-8 rounded-full flex items-center justify-center text-[10px] md:text-xs shadow-lg shadow-indigo-500/30">1</span> 
                        Pilih Jenis Layanan
                    </h3>
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-5">
                        ...
                        <label class="relative cursor-pointer group">
                            ...
                            <div class="border-2 border-slate-200 bg-white/60 rounded-[1.5rem] md:rounded-[2rem] p-5 md:p-6 hover:border-indigo-400 hover:bg-white transition-all h-full flex flex-col justify-between">
                                <div>
                                    <div class="flex justify-between items-start mb-3 gap-2">
                                        ...
                                        <div class="w-5 h-5 rounded-full border-2 border-slate-300 flex items-center justify-center mt-1 group-hover:border-indigo-400 bg-white shrink-0">
                                            <div class="radio-dot w-2.5 h-2.5 bg-indigo-600 rounded-full opacity-0 transform scale-0 transition-all duration-300"></div>
                                        </div>
                                    </div>
                                    <p class="text-[10px] md:text-[11px] text-slate-500 font-medium mb-5 leading-relaxed line-clamp-3">{{ $l->deskripsi }}</p>
                                </div>
                                <p class="text-indigo-600 font-black text-base md:text-lg tracking-tighter">Rp {{ number_format($l->harga, 0, ',', '.') }} <span class="text-[9px] md:text-[10px] text-slate-400 font-bold uppercase tracking-widest not-italic">/ Ps</span></p>
                            </div>
                        </label>
                        @endforeach
                    </div>
                ...
                <div>
                    <h3 class="font-black text-base md:text-lg uppercase tracking-widest mb-6 flex items-center gap-3 text-slate-900">
                        <span class="bg-indigo-600 text-white w-7 h-7 md:w-8 md:h-8 rounded-full flex items-center justify-center text-[10px] md:text-xs shadow-lg shadow-indigo-500/30">2</span> 
                        Metode Penyerahan
                    </h3>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-5">
                        <label class="relative cursor-pointer group">
                            ...
                            <div class="border-2 border-slate-200 bg-white/60 rounded-[1.5rem] md:rounded-[2rem] p-6 md:p-8 hover:border-indigo-400 hover:bg-white transition-all h-full">
                                <div class="flex justify-between items-start mb-2">
                                    <h4 class="font-black text-lg md:text-xl text-slate-800 italic tracking-tight transition-colors">Antar ke Toko</h4>
                                    <div class="w-5 h-5 rounded-full border-2 border-slate-300 flex items-center justify-center group-hover:border-indigo-400 bg-white shrink-0">
                                        <div class="radio-dot w-2.5 h-2.5 bg-indigo-600 rounded-full opacity-0 transform scale-0 transition-all duration-300"></div>
                                    </div>
                                </div>
                ...
                <div>
                    <h3 class="font-black text-base md:text-lg uppercase tracking-widest mb-6 flex items-center gap-3 text-slate-900">
                        <span class="bg-indigo-600 text-white w-7 h-7 md:w-8 md:h-8 rounded-full flex items-center justify-center text-[10px] md:text-xs shadow-lg shadow-indigo-500/30">3</span> 
                        Detail Pesanan
                    </h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 items-start">
                        <div>
                            ...
                            <input type="number" id="jumlah_sepatu" name="jumlah_sepatu" min="1" value="{{ old('jumlah_sepatu', 1) }}" required 
                                   class="w-full bg-white border border-slate-200 text-slate-900 text-lg rounded-[1.5rem] p-4 md:p-5 font-black focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all shadow-sm">
                        </div>
                        
                        <div id="areaAlamat" class="hidden transition-all duration-300">
                            <label class="block text-[9px] font-black text-indigo-600 uppercase tracking-[0.2em] mb-3 ml-2">Alamat Penjemputan</label>
                            <textarea name="alamat_jemput" id="alamat_jemput" rows="2" placeholder="Masukkan alamat lengkap penjemputan..." 
                                      class="w-full bg-blue-50 border border-blue-200 text-slate-900 text-sm rounded-[1.5rem] p-4 md:p-5 font-medium focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-600 outline-none resize-none transition-all shadow-sm">{{ old('alamat_jemput') }}</textarea>
                        </div>
                    </div>
                </div>

                {{-- STEP 4: PEMBAYARAN (QRIS / MIDTRANS) --}}
                <div>
                    <h3 class="font-black text-base md:text-lg uppercase tracking-widest mb-6 flex items-center gap-3 text-slate-900">
                        <span class="bg-indigo-600 text-white w-7 h-7 md:w-8 md:h-8 rounded-full flex items-center justify-center text-[10px] md:text-xs shadow-lg shadow-indigo-500/30">4</span> 
                        Metode Pembayaran
                    </h3>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-5">
                        <label class="relative cursor-pointer group">
                            ...
                            <div class="border-2 border-slate-200 bg-white/60 rounded-[1.5rem] md:rounded-[2rem] p-6 md:p-8 hover:border-indigo-400 hover:bg-white transition-all h-full">
                                <div class="flex justify-between items-start mb-2">
                                    <h4 class="font-black text-lg md:text-xl text-slate-800 italic tracking-tight transition-colors">Bayar di Kasir</h4>
                                    <div class="w-5 h-5 rounded-full border-2 border-slate-300 flex items-center justify-center group-hover:border-indigo-400 bg-white shrink-0">
                                        <div class="radio-dot w-2.5 h-2.5 bg-indigo-600 rounded-full opacity-0 transform scale-0 transition-all duration-300"></div>
                                    </div>
                                </div>
                                <p class="text-[10px] md:text-[11px] text-slate-500 font-medium">Tunai/QRIS langsung saat serah-terima di bengkel.</p>
                            </div>
                        </label>
                        ...
                        <label class="relative cursor-pointer group">
                            ...
                            <div class="border-2 border-slate-200 bg-white/60 rounded-[1.5rem] md:rounded-[2rem] p-6 md:p-8 hover:border-indigo-400 hover:bg-white transition-all h-full">
                                ...
                                <p class="text-[10px] md:text-[11px] text-slate-500 font-medium">Pembayaran instan via QRIS, E-Wallet, atau Virtual Account.</p>
                            </div>
                        </label>
                    </div>

                    {{-- Info Box Midtrans --}}
                    <div id="areaTransfer" class="hidden mt-6 bg-cyan-50/80 p-6 md:p-8 rounded-[2rem] border border-cyan-100 transition-all text-center">
                        <img src="https://upload.wikimedia.org/wikipedia/commons/e/e0/Midtrans.png" class="h-6 md:h-8 mx-auto mb-4" alt="Midtrans">
                        <h4 class="font-black text-slate-800 uppercase tracking-widest text-xs md:text-sm mb-2">Sistem Pembayaran Terotomatisasi</h4>
                        <p class="text-[10px] md:text-[11px] text-slate-500 font-medium max-w-lg mx-auto">
                    ...
                    <div id="areaBayarToko" class="hidden mt-6 bg-slate-50 p-6 md:p-8 rounded-[2rem] border border-slate-200 transition-all">
                        <div class="flex flex-col md:flex-row items-start gap-5">
                            <div class="w-12 h-12 rounded-full bg-white border border-slate-200 flex items-center justify-center shrink-0 shadow-sm text-indigo-600">
                ...
                <div class="bg-blue-50/50 border border-blue-100 p-8 md:p-10 rounded-[2rem] flex flex-col md:flex-row justify-between items-center gap-6 mt-6 shadow-sm">
                    <div class="text-center md:text-left">
                        ...
                        <h2 class="text-4xl md:text-5xl font-black text-indigo-600 italic tracking-tighter" id="totalTagihan">Rp 0</h2>
                    </div>
                    <button type="submit" id="btnSubmit" class="w-full md:w-auto px-12 lg:px-16 py-5 lg:py-6 bg-indigo-600 text-white font-black uppercase text-[10px] md:text-xs tracking-[0.3em] rounded-[1.5rem] shadow-lg shadow-indigo-500/30 active:scale-95 transition-all hover:bg-slate-900 hover:shadow-slate-900/30 flex items-center justify-center gap-3">
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            form.addEventListener('submit', function() {
                setTimeout(() => {
                    ...
                    btnSubmit.classList.remove('shadow-indigo-500/30');
                }, 50);
            });
