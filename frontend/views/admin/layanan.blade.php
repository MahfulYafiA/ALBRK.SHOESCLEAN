...
<body class="text-slate-200 antialiased flex h-screen overflow-hidden relative selection:bg-{{ $accent }}-500 selection:text-white">
                            ...
                            <tr class="hover:bg-slate-800/30 transition-all">
                                <td class="px-4 py-3">
                                    <div class="flex justify-center">
                                        @if($l->gambar)
                                            <img src="{{ route('media.public', ['path' => $l->gambar]) }}" class="w-12 h-12 object-cover rounded-lg shadow border border-slate-700 cursor-pointer hover:ring-2 hover:ring-{{ $accent }}-500 transition-all" onclick="openEditModal('{{ $l->id_layanan }}', '{{ addslashes($l->nama_layanan) }}', {{ $l->harga }}, '{{ addslashes($l->deskripsi ?? '') }}', '{{ $l->gambar ? route('media.public', ['path' => $l->gambar]) : '' }}')">
                                        @else
                                            <button onclick="openEditModal('{{ $l->id_layanan }}', '{{ addslashes($l->nama_layanan) }}', {{ $l->harga }}, '{{ addslashes($l->deskripsi ?? '') }}', '')" class="w-12 h-12 bg-slate-800 border border-slate-700 rounded-lg flex items-center justify-center text-slate-500 hover:bg-{{ $accent }}-500/20 hover:border-{{ $accent }}-500 hover:text-{{ $accent }}-400 transition-all">
                                ...
                                <td class="px-4 py-3 font-bold text-{{ $accent }}-400 whitespace-nowrap">Rp {{ number_format($l->harga, 0, ',', '.') }}</td>
                                <td class="px-4 py-3">
                                    <div class="flex justify-end gap-1.5">
                                        <button onclick="openEditModal('{{ $l->id_layanan }}', '{{ addslashes($l->nama_layanan) }}', {{ $l->harga }}, '{{ addslashes($l->deskripsi ?? '') }}', '{{ $l->gambar ? route('media.public', ['path' => $l->gambar]) : '' }}')" class="w-8 h-8 rounded-lg bg-slate-800 text-slate-400 hover:bg-{{ $accent }}-600 hover:text-white border border-slate-700 hover:border-transparent transition-all flex items-center justify-center">
                                            <i class="fa-solid fa-pen text-xs"></i>
                                        </button>
                                        <form action="{{ route('admin.layanan.destroy', $l->id_layanan) }}" method="POST" onsubmit="return confirm('Hapus layanan ini beserta foto?');" class="m-0">
                                            @csrf @method('DELETE')
    ...
    <script>
        ...
        // ✅ TAMBAHAN: FUNGSI PREVIEW UNTUK TENTANG KAMI
