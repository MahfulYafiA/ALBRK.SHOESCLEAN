            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6 mb-8 md:mb-10 relative z-10">
                ...
                <div class="glass-panel relative overflow-hidden p-6 md:p-8 rounded-[2rem] flex flex-col justify-center transition-all border-amber-500/30">
                    ...
                    <div class="relative z-10">
                        ...
                        <h3 class="text-3xl md:text-4xl font-black text-amber-400 italic tracking-tighter">{{ $reservasis->whereIn('status', ['Diajukan', 'Menunggu Konfirmasi'])->count() }}</h3>
                    </div>
                </div>
                <div class="glass-panel p-6 md:p-8 rounded-[2rem] flex flex-col justify-center transition-all hover:border-emerald-500/50 hover:bg-slate-800/80 group">
                    ...
                    <h3 class="text-3xl md:text-4xl font-black text-emerald-500 italic tracking-tighter">{{ $reservasis->where('status', 'Selesai')->count() }}</h3>
                </div>
            </div>
                        ...
                        <tbody class="divide-y divide-slate-800/50 text-sm">
                            ...
                            @php
                                ...
                                $statusClass = match($p->status) {
                                    ...
                                    'Diproses' => 'bg-indigo-500/20 text-indigo-400 border-indigo-500/30',
                                    ...
                                    <div class="flex flex-col gap-2 items-start">
                                        ...
                                        @if($p->pembayaran && $p->pembayaran->bukti_pembayaran)
                                            <button onclick="openImageModal('{{ asset('storage/' . $p->pembayaran->bukti_pembayaran) }}')" 
                                                class="text-indigo-400 hover:text-blue-300 transition-colors font-black text-[9px] uppercase tracking-widest flex items-center gap-1.5 px-1 py-0.5">
    ...
    <script>
        function showDetail(id, nama, layanan, jumlah, alamat, status, metode, noTelp) {
            ...
            if(noTelp) {
                ...
                waLink = "https://wa.me/" + formattedNumber + "?text=Halo%20Kak%20" + encodeURIComponent(nama) + ",%20saya%20Admin/Kurir%20dari%20ALBRK.SHOECLEAN...%20";
            }
